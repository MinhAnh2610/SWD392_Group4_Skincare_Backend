...
public class CouponController : ICarterModule
{
  public void AddRoutes(IEndpointRouteBuilder app)
  {
    ...
    group.MapPost("/exchange", async (ICouponService couponService, [FromBody] ExchangeCouponRequest request) =>
      {
        var result = await couponService.ExchangeCouponAsync(request);
        return result.Match("Coupon exchanged successfully.");
      })
      .WithName("ExchangeCoupon")
      .Produces<ApiResponse<CouponResponse>>(StatusCodes.Status200OK)
      .ProducesProblem(StatusCodes.Status400BadRequest)
      .RequireAuthorization();

    group.MapGet("/game", async (ICouponService couponService) =>
      {
        var result = await couponService.StartGameAsync();
        return result.Match("Redirecting to game.");
      })
      .WithName("StartGame")
      .Produces<ApiResponse<GamePointResponse>>(StatusCodes.Status200OK)
      .ProducesProblem(StatusCodes.Status400BadRequest)
      .RequireAuthorization();
    
    group.MapPost("/game", async (ICouponService couponService, [FromBody] GamePointRequest request) =>
      {
        var result = await couponService.ProcessGamePoints(request);
        return result.Match("Points exchanged successfully.");
      })
      .WithName("ExchangeGamePoints")
      .Produces<ApiResponse<GamePointResponse>>(StatusCodes.Status200OK)
      .ProducesProblem(StatusCodes.Status400BadRequest)
      .RequireAuthorization();
