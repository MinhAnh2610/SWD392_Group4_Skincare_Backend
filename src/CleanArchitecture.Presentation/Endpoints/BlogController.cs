...
public class BlogController : ICarterModule
{
  public void AddRoutes(IEndpointRouteBuilder app)
  {
    ...
    group.MapGet("/",
        async (IBlogService service, string? title, string? content, string[]? tags, string? staffUsername,
          string? sortColumn,
          string? sortOrder = "asc", int pageIndex = 1, int pageSize = 10) =>
        {
          var request = new GetProductRequest(title, content, sortColumn, sortOrder, tags, staffUsername, pageIndex,
            pageSize);
    ...
    group.MapPut("/{id}",
        async (IBlogService service, [FromRoute] Guid id, [FromBody] UpdateBlogRequest updateRequest) =>
        {
